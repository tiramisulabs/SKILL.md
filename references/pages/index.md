...
One Markdown note per supplied docs URL. Each note has a page summary, **source-verified** key APIs and code examples, an explicit **Doc vs Source Corrections** section, the `seyfert-core` source anchors used, and agent guidance. Verified against `seyfert-core` branch `more-qol`; cross-cutting corrections live in `../source-truth.md`.
