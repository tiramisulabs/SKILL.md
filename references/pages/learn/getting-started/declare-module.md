...
- `src/index.ts` (root exports, `extendContext`, `config`, `createPlugin`, `definePlugins`)
- `src/client/plugins/types.ts` (`SeyfertRegistry`, `RegisteredPlugins`, `RegisteredPlugin{Extension,Context,Middlewares}`)
- `src/client/plugins.ts` (`createPlugin`, `createPluginFactory`, `definePlugins`)
- `src/commands/applications/shared.ts` (`UsingClient`, `ParseClient`, `ParseGlobalMiddlewares`, `GlobalMetadata`, `ExtendContext`, `ExtraProps`, `InternalOptions`, `InferWithPrefix`, `ExtendedRC`, `ExtendedRCLocations`, `DefaultLocale`, `CustomStructures`)
- `src/commands/decorators.ts` (`RegisteredMiddlewares`, `ResolvedRegisteredMiddlewares`, `InferMiddlewares`)
- `src/langs/router.ts` (`ParseLocales`)
- `src/cache/index.ts` (`InferAsyncCache`, `ReturnCache`)
- `src/client/base.ts` (`RC`, `RCLocations`, `ResolvedRC`, `getRC`)
- `src/client/transformers.ts` (`InferCustomStructure`)
- `src/client/index.ts` (`Client`, `HttpClient`, `WorkerClient` exports)

## Agent Guidance

- One `declare module 'seyfert'` block in `src/index.ts` is the convention; it must be inside `tsconfig.json` include scope or types silently drop to defaults (`{}` / `BaseClient` / `false`).
- Always brand the client with `ParseClient<...>` — a bare client type to `SeyfertRegistry.client` will not resolve `UsingClient` correctly.
- Do NOT augment `UsingClient`, `RegisteredMiddlewares`, or `DefaultLocale` directly in v5 — register through `SeyfertRegistry`.
- `withPrefix: true` makes both `ctx.interaction` and `ctx.message` optional; only flip it on if you run text/prefix commands.
- `asyncCache: true` is required when the cache adapter returns promises (e.g. `@slipher/redis-adapter`, EXTERNAL — verify version in target project); it changes `ReturnCache<T>` to `Promise<T>` everywhere.
- Register plugins under `SeyfertRegistry.plugins` so their client/context/middleware extensions are typed — you no longer hand-write those augmentations.
- Read extended config at runtime via `await client.getRC()`; extended folder paths live under `rc.locations`.
- Remember `start()` does not upload commands — call `uploadCommands(...)` after start.
