...
- `createEvent({ data: { name, once? }, run })` — `src/index.ts:68`. `once` defaults to `false`. `run` is typed `Awaitable<void>`. Gateway events still receive `(payload, client, shardId)`; v5 dropped the trailing `shardId` only for CUSTOM (non-gateway) events.
