...
- EXTERNAL (yunaforseyfert, version-verify): `Yuna.plugin({ parser, resolver })`, `Yuna.watchers.find(client, { userId, command })` -> watcher with `.stop(reason)`, `@Watch({ idle, beforeCreate })`, and the injected `client.yuna` / `ctx.yuna` accessors. None of these exist in seyfert-core.
...
- GOTCHA: `Yuna.plugin`, `@Watch`, `Yuna.watchers`, and `client.yuna`/`ctx.yuna` are NOT in seyfert-core — they come from `yunaforseyfert`. Their option names are doc-authoritative; tell users to verify the installed `yunaforseyfert` version. Everything under `seyfert` root imports (`definePlugins`, `ParseClient`, `commands.prefix/reply`, `InternalOptions`, `SeyfertRegistry`, `Command`/`Declare`/`Options`/`create*Option`) IS source-verified above.
