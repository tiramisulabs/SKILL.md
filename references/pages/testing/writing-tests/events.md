...
- `src/events/hooks/dispatch.ts:10` — `READY` transformer (`ClientUserStructure`)
- `src/structures/Message.ts:164` — `Message.reply(body, fail?)`
- `src/common/shorters/users.ts:46-47` — `users.write` (createDM → messages.write)

## Agent Guidance

- When writing/editing an event handler a test feeds, keep `data.name` in **camelCase** (the `ClientNameEvents` key). `data: { name: 'GUILD_MEMBER_ADD' }` is wrong for `createEvent`.
- `emit` uses the **uppercase** gateway dispatch name; a mis-cased name throws loud (no-handler) unless `allowNoHandler: true` is passed — use that flag only for pure world-state seeding.
- Emit **full** Discord event shapes, not partial patches.
- For gateway events the `run` signature is `(payload, client, shardId)`; for custom events it is `(payload, client)` with no `shardId` (v5).
- `createEvent` returns the input object (with `once` defaulted to `false`); passing the default export straight into `events: [...]` is correct.
- `client.users.write` performs a real `createDM` round-trip in production; in tests the toolkit records it so you read it back via `bot.world.query.dm(...)`.
- Toolkit (`@slipher/testing`) APIs are external: confirm method names/options against the version installed in the consuming project before relying on them.
